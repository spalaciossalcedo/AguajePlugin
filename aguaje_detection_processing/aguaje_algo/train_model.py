...
Date: 2019
Availability: https://github.com/pgbrodrick/ecoCNN"""

import numpy as np
import sys,os,shutil
import time

from ..aguaje_algo import network
#from util import *
from ..aguaje_algo.util import *
...
def load_trained_model(savename,window_radius,internal_window_radius=None,save_directory='trained_models',weighted=False,verbose=True):
  ...
  savename_base = os.path.join(save_directory,savename)
  print('File: ' + savename_base + '.json')

  try:
    jf = open(savename_base + '.json','r')
    mymodel = model_from_json(jf.read())
    jf.close()
  except:
  ...
  try:
    mymodel.load_weights(savename_base + '_weights')
    print('Load model weight file: ' + savename_base + '_weights')
  except:
    print('Could not load model weight file: ' + savename_base + '_weights')
    return None

  try:
    mymodel.compile(loss=network.cropped_categorical_crossentropy(window_radius*2,internal_window_radius*2,weighted=weighted),optimizer='adam')
  except:
    print('Could not compile model with given window_radius, internal_window_radius, and weighted flag')
    return None

  if (verbose): mymodel.summary()
